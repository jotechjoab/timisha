  ...
  <!-- Main content -->
   <section class="content" id="pdf">
        ...
        <div class="col-xs-12">
          ...
          <h3 style="text-align: center;"> Restaurant Sales Report From <?php echo date("d  F, Y",strtotime($_POST['from']))." To ".date("d  F, Y",strtotime($_POST['to'])); ?></h3>
        </div>
        <!-- /.col -->
      </div>
     <div class="row">
...
$total_upaid=0;
if ($type=='ind_sales') {
$sql="SELECT i.details_grp_id,t.trans_date,p.id as item_code,p.name,d.quantity,d.rate,u.fname,u.lname,(d.quantity*d.rate) as totalprice,i.payment_status,p.dept FROM users u,menu_items p,invoice_details d, invoices i,transactions t WHERE  ((u.id=d.created_by AND p.id=d.pdt_code) AND (d.details_grp_id=i.details_grp_id)) AND((t.created_at=i.created_at))AND ( d.created_at BETWEEN '$datefrom' AND '$dateto') AND u.id='$id' AND p.dept='$dept'   GROUP BY d.id ORDER BY `t`.`trans_date`  DESC";

}else{
 $sql="SELECT i.details_grp_id,t.trans_date,p.id as item_code,p.name,d.quantity,d.rate,u.fname,u.lname,(d.quantity*d.rate) as totalprice,i.payment_status,p.dept FROM users u,menu_items p,invoice_details d, invoices i,transactions t WHERE  ((u.id=d.created_by AND p.id=d.pdt_code) AND (d.details_grp_id=i.details_grp_id)) AND((t.created_at=i.created_at))AND ( d.created_at BETWEEN '$datefrom' AND '$dateto') AND p.dept='$dept'  GROUP BY d.id
