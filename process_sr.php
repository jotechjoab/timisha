       ...
       <div class="box">
        ...
        <div class="box-body">
         ...
         <input type="hidden" name="id" id="id" value="<?php echo $info['id'];?>"> 
          ...
          $sr=$_GET['sr'];
          $dept=$_GET['dept'];
        $sql="SELECT d.id,p.item_code,p.name,p.description,d.quantity,d.rate,d.unit_of_measure,p.supplier_id FROM products p , sr_details d WHERE (d.pdt_code=p.item_code AND d.details_grp_id='$sr') AND  d.status='Not Cleared' ";
        ...
        if (mysqli_num_rows($query)>0) {
          ...
          while ($row=mysqli_fetch_array($query)) {
            ...
            $inv="SELECT sum(qty_in_store) as sum FROM inventories WHERE item_code='$item_code'";
            $in_store=mysqli_fetch_array(mysqli_query($con,$inv));
            if ($in_store['sum']>0) {   
              if ($in_store['sum']>=$row['quantity']) {
     $process=' <a class="btn btn-primary" href="to_stock.php?item_code='.$item_code.'&sr='.$sr.'&qty='.$row['quantity'].'&in_store='.$in_store['sum'].'&dept='.$dept.'">Stock</a>';
            }else{
           $process='<a class="btn btn-success" href="sr_to_po.php?item_code='.$item_code.'&qty='.$row['quantity'].'&supplier='.$row['supplier_id'].'&id='.$info['id'].'&uom='.$row['unit_of_measure'].'&rate='.$row['rate'].'&sr='.$sr.'">Purchase</a>';
            }
            }else{
              $process=' <button class="btn btn-success">Purchase</button>';
            }
